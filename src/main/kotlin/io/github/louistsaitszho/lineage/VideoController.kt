...
//    }
    ...
    @GetMapping("/rest/v1/schools")
    fun getSchools(@RequestHeader(value = "access_token") accessToken: String): Response<SchoolAttributes> {
        connection?.autoCommit = false
        //access token
        psGetSchoolByToken?.setString(1, accessToken)

        val rs = psGetSchoolByToken?.executeQuery()
        val dataList = mutableListOf<Data<SchoolAttributes>>()
        while (rs?.next() == true) {
            val id = rs.getString("id")
            val type = rs.getString("type")
            val name = rs.getString("name")
            dataList.add(Data(id, type, SchoolAttributes(name)))
        }
        return Response(dataList, null)
    }
}
